...
(function()
 {
     ...
     var EVENTS = ormma.EVENTS = {
         ...
         MRAIDREADY : 'ready',
         MRAIDVIEWABLECHANGE : 'viewableChange'
	 };
     ...
     var changeHandlers = {
         state : function(val)
         {
             ...
             if (state == STATES.DEFAULT) {
                 console.log('TRYING TO SIGNAL ormma to ready');
                 ...
                 broadcastEvent(EVENTS.MRAIDREADY,
                                'controller initialized, attempting callback');
			 }
         ...
         viewable : function(val)
         {
             ...
             viewable = val;
             broadcastEvent(EVENTS.MRAIDVIEWABLECHANGE, viewable);
		 }
	 };
     var listeners = {};
     ...
     ormmaview.addEventListener('ready', function(properties)
                            {
                                broadcastEvent(EVENTS.MRAIDREADY, 'controller initialized, attempting callback');
     ...
     var broadcastEvent = function()
     {
         ...
         if (event == EVENTS.ERROR) {
             ormmaview.onError(args[0], args[1]);
		 }
         try {
             if (listeners[event])
     ...
     ormma.addEventListener = function(event, listener)
     {
         if (!event || !listener) {
             ...
             if (event == EVENTS.LOCATIONCHANGE) {
                 window.ormmaview.startLocationListener();
			 }
		 }
	 };
     ...
     {
         broadcastEvent(EVENTS.INFO, 'expanding');
         var dimensions = {
             x : 0,
             y : 0,
             width : ormmaview.getExpandProperties().width == 0 ? maxSize.width
			 : ormmaview.getExpandProperties().width,
             height : ormmaview.getExpandProperties().height == 0 ? maxSize.height
			 : ormmaview.getExpandProperties().height,
		 };
     ...
     ormma.removeEventListener = function(event, listener)
     {
         if (!event) {
             ...
             if (listener
                 ...
                 if (event == EVENTS.LOCATIONCHANGE) {
                     window.ormmaview.stopLocationListener();
				 }
			 }
     ...
     {
         tempProperties = ormmaview.getExpandProperties();

         if (properties.useBackground != undefined) {
             if (properties.useBackground === true || properties.useBackground === false) {
                 tempProperties.useBackground = properties.useBackground;
			 }
		 }

         if (properties.backgroundColor != undefined) {
             if (typeof properties.backgroundColor == 'string' && properties.backgroundColor.substr(0, 1) == '#' && !isNaN(parseInt(
                                                                                                                               properties.backgroundColor.substr(1), 16))) {
                 tempProperties.backgroundColor = properties.backgroundColor;
			 }
		 }

         if (properties.backgroundOpacity != undefined) {
             if (!isNaN(properties.backgroundOpacity) && properties.backgroundOpacity >= 0 && properties.backgroundOpacity <= 1) {
                 tempProperties.backgroundOpacity = properties.backgroundOpacity;
			 }
		 }

         if (properties.lockOrientation != undefined) {
             if (properties.lockOrientation === true || properties.lockOrientation === false) {
                 tempProperties.lockOrientation = properties.lockOrientation;
			 }
		 }

         if (properties.useCustomClose != undefined) {
             if (properties.useCustomClose === true || properties.useCustomClose === false) {
                 tempProperties.useCustomClose = properties.useCustomClose;

                 ormma.useCustomClose(properties.useCustomClose);
			 }
		 }

         if (properties.width != undefined) {
             if (typeof properties.width == 'number' && properties.width > 0) {
                 tempProperties.width = properties.width;
			 }
		 }
         if (properties.height != undefined) {
             if (typeof properties.height == 'number' && properties.height > 0) {
                 tempProperties.height = properties.height;
			 }
		 }

         ormmaview.setExpandProperties(tempProperties);
	 };
    ...
    ormma.getOrientationProperties = function()
 {
    return (ormmaview.getOrientationProperties());
     ...
     ormma.setOrientationProperties = function(properties)
        {
            tempProperties = ormmaview.getOrientationProperties();
 
            if (properties.allowOrientationChange != undefined) {
                if (properties.allowOrientationChange === true || properties.allowOrientationChange === false) {
                    tempProperties.allowOrientationChange = properties.allowOrientationChange;
                }
            }
 
            if (properties.forceOrientation != undefined) {
                if (properties.forceOrientation == 'string') {
                    // portrait|landscape|none MRAID 2.0
                    tempProperties.forceOrientation = properties.forceOrientation;
                }
            }
            ormmaview.setOrientationProperties(tempProperties);
        };
	 ...
	 ormma._createCalendarEvent = function(parameters)
	 {
		 if (!supports[FEATURES.CALENDAR]) {
			 broadcastEvent(EVENTS.ERROR,
			                'Method not supported by this client.', 'createCalendarEvent');
		 }
		 else if (!parameters || typeof parameters != 'object') {
			 broadcastEvent(EVENTS.ERROR, 'Valid parameters required.', 'createEvent');
		 }
		 else {
			 var description = parameters.description;
			 var summary = parameters.summary;
			 var start = parameters.start;
			 var end = parameters.end;
			 var location = parameters.location;
 
			 ormmaview.createCalendarEvent(start, end, description, summary, location);
		 }
	 };
     ...
     ormma.getLocation = function()
     {
         ...
         return (null == location) ? window.ormmaview.getLocation()
		 : clone(location);
	 };
     ...
     ormma.getAssetURL = function(alias)
     {
         ...
         return ormmaview.getAssetURL(alias);
	 };
