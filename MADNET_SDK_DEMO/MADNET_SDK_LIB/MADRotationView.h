...
/**
 Максимальные размеры, которые может принимать контейнер.
 По умолчанию равны размеру экрана.
 */
...
/**
 Служебные методы
 */
- (void) _testRequestWithURLString: (NSString *) aURLString;

- (BOOL) loadCreative: (NSURL *)url;

- (BOOL) loadHTMLCreative: (NSString *)htmlFragment
			  creativeURL: (NSURL *)url;

- (BOOL) requestAdWithSpecialHandlerPrefix: (NSString *) aPrefix;
